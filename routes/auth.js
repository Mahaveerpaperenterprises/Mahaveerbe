...
router.post('/signup', async (req, res) => {
  ...
  try {
    ...
    // insert; rely on unique index LOWER(email) to prevent dupes
...
/* FORGOT PASSWORD (Users table only) */
  ...
  try {
    ...
    if (!user) {
      if (SHOW_404_WHEN_NOT_FOUND) {
        ...
        // Don’t reveal existence (prod choice). Frontend can ignore requestId.
    ...
    return res.json({ ok: true, requestId: `user_${user.id}` });
  } catch (err) {
    console.error(err);
 ...
 * We re-check the OTP here as well for safety .
 */
